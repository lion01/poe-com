...
class PoecomModelAddress extends JModelForm{
    ...
    */
    public function getForm($data = array(),$loadData = true, $address_type = 'BT'){
        ...
        if(isset($data['address_type']) && $data['address_type']  == 'ST'){
    ...
    */
    public function getScript(){
        return 'components/com_poecom/models/forms/address.js';
    }
    ...
    */
    ...
    /**
     * Check user address found a submitted address
     * 
     * @param array $address
     * @return int $address_id
     */
    public function checkUniqueAddressId($address = array()){
        $db = $this->getDbo();
        $q = $db->getQuery(true);
        $q->select('id');
        ...
        if(!empty($address['street1'])){
            $q->where('street1=' . $db->quote($address['street1']));
        }
        if(!empty($address['city'])){
            $q->where('city=' . $db->quote($address['city']));
        }
        if(!empty($address['region_id'])){
            $q->where('region_id=' . (int)$address['region_id']);
        }
        if(!empty($address['country_id'])){
            $q->where('country_id=' . (int)$address['country_id']);
        }
