 ...
 * FlyerBlocks helper.
 */
abstract class FlyerBlocksHelper {
    ...
    public static function addSubmenu($submenu) {
        ...
        $document->addStyleDeclaration('.icon-48-flyerblocks {background-image: url(../media/com_poecom/images/icon-48-flyers.png);}');
    }

    /**
     * Get the actions
     */
    public static function getActions($flyerBlockId = 0) {
        jimport('joomla.access.access');
        $user = JFactory::getUser();
        $result = new JObject;

        if (empty($flyerBlockId)) {
            $assetName = 'com_poecom';
        } else {
            $assetName = 'com_poecom.name.' . (int) $flyerBlockId;
        }
