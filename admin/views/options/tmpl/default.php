...
<form action="<?php echo JRoute::_('index.php?option=com_poecom'); ?>" method="post" name="adminForm" id="adminForm">
    <fieldset id="filter-bar">
        <div class="filter-search fltlft">
            ...
            <input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" title="<?php echo JText::_('COM_POECOM_OPTION_SEARCH'); ?>" />
                ...
                <th>
                    <?php echo JHtml::_('grid.sort', 'JGRID_HEADING_ORDERING', 'ordering', $listDirn, $listOrder);
                        if ($canOrder && $saveOrder) {
                                echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'options.saveorder');
                        } ?>
                </th>
                <th>
            ...
            if ($this->items) {
                foreach ($this->items as $i => $item) {
                    ...
                    <tr class="row<?php echo $i % 2; ?>">
                        ...
                        <td>
                            <a href="index.php?option=com_poecom&view=option&task=option.edit&id=<?php echo $item->id; ?>"><?php echo $item->name; ?></a>
                        </td>
                        <td>
                        ...
                        <td class="order">
                            ...
                            if ($canChange) {
                                if ($saveOrder) {
                                    if ($listDirn == 'asc') {
                                        ?>
                                        <span><?php echo $this->pagination->orderUpIcon($i, (@$this->items[$i - 1]->product_id == $item->product_id), 'options.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                        <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, (@$this->items[$i + 1]->product_id == $item->product_id), 'options.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                    <?php } elseif ($listDirn == 'desc') { ?>
                                        <span><?php echo $this->pagination->orderUpIcon($i, (@$this->items[$i - 1]->product_id == $item->product_id), 'options.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
                                        <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, (@$this->items[$i + 1]->product_id == $item->product_id), 'options.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
                                    <?php
                                    }
                                }
