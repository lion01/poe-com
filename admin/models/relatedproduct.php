 ...
 */
class PoecomModelRelatedProduct extends JModelAdmin {
    /**
     ...
     * @since	1.6
     */
    public function getTable($type = 'RelatedProduct', $prefix = 'PoecomTable', $config = array()) {
    ...
    public function getForm($data = array(), $loadData = true) {
        // Get the form.
        $form = $this->loadForm('com_poecom.relatedproduct', 'relatedproduct', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        return $form;
    ...
    protected function loadFormData() {
        ...
        $data = JFactory::getApplication()->getUserState('com_poecom.relatedproduct.edit.data', array());
        if (empty($data)) {
            $data = $this->getItem();
        }
        return $data;
    ...
    /**
     * Check if product id exists in related products
    ...
    public function validateProductDelete($product_id = 0){
        ...
        if(($id = $db->loadResult())){
            $this->setError('Related Product : ' . $id);
            return false;
        }else{
            return true;
        }
