 ...
 * FlyerRow Model
 */
class PoecomModelFlyerRow extends JModelAdmin {
     ...
     * @since	1.6
     */
    public function getTable($type = 'FlyerRow', $prefix = 'PoecomTable', $config = array()) {
    ...
    */
    public function getScript(){
        $file = JPATH_ADMINISTRATOR.'/components/com_poecom/models/forms/flyerrow.js';
 
        if(JFile::exists($file)){
            $js = JURI::root(true).'/administrator/components/com_poecom/models/forms/flyerrow.js';
        }else{
            $js = '';
        }
    ...
    public function getForm($data = array(), $loadData = true) {
        // Get the form.
        $form = $this->loadForm('com_poecom.flyerrow', 'flyerrow', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        return $form;
    ...
    protected function loadFormData() {
        ...
        $data = JFactory::getApplication()->getUserState('com_poecom.flyerrow.edit.data', array());
        if (empty($data)) {
            $data = $this->getItem();
        }
        return $data;
   ...
   protected function getReorderConditions($table){
           ...
           $condition[] = 'section_id = '. $this->_db->Quote($table->section_id);
    ...
    public function resetSort($id = 0, $group = '', $pos = 0, $ordering = 0){
        ...
        $q->from('#__poe_flyer_row');
        if(!empty($group)){
            $q->where('section_id='.$db->quote($group));
        }
        $db->setQuery($q);
