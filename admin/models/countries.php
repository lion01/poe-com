...
class PoecomModelCountries extends JModel {
    ...
    public function getRegions($country_id = 0) {
        ...
        if( ($regions = $this->_db->loadObjectList()) ){
            $this->regions[] = array('value' => 'ALL', 'text' => JText::_('COM_POECOM_S_SHOW_ALL_REGIONS'));
            foreach($regions as $r){
                $this->regions[] = array('value' => $r->value, 'text' => $r->text);
            }
        }else{
            $this->regions[] = array('value' => 0, 'text' => JText::_('COM_POECOM_NO_REGIONS'));
        }
