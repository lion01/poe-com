 ...
 * FlyerBlock Model
 */
class PoecomModelFlyerBlock extends JModelAdmin {
     ...
     * @since	1.6
     */
    public function getTable($type = 'FlyerBlock', $prefix = 'PoecomTable', $config = array()) {
    ...
    */
    public function getScript(){
        ...
        $file = JPATH_ADMINISTRATOR.'/components/com_poecom/models/forms/flyerblock.js';
 
        if(JFile::exists($file)){
            $js = JURI::root(true).'/administrator/components/com_poecom/models/forms/flyerblock.js';
        }else{
            $js = '';
        }
    ...
    public function getForm($data = array(), $loadData = true) {
        // Get the form.
        $form = $this->loadForm('com_poecom.flyerblock', 'flyerblock', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        return $form;
    ...
    protected function loadFormData() {
        ...
        $data = JFactory::getApplication()->getUserState('com_poecom.flyerblock.edit.data', array());
        if (empty($data)) {
            $data = $this->getItem();
        }
        return $data;
    ...
    /**
     * Check if product id exists in order lines
