 ...
 * Currency Model
 */
class PoecomModelCurrency extends JModelAdmin{
    /**
    ...
    */
    public function getForm($data = array(), $loadData = true){
        // Get the form.
        $form = $this->loadForm('com_poecom.currency', 'currency', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)){
            return false;
        }
        return $form;
    ...
    */
    protected function loadFormData(){
        ...
        $data = JFactory::getApplication()->getUserState('com_poecom.currency.edit.data', array());
        if (empty($data)){
                $data = $this->getItem();
        }
        return $data;
    ...
    */
    ...
    public function delete(&$cids){
        if($cids){
            ...
            foreach($cids as $cid){
                //check if used 
                //TODO: add currency delete checks
                $used = true;
                
                //check status not (2)paided and there is no link to an rfq or order
                if($used){
		    JError::raiseWarning(500, JText::_('COM_POECOM_COUPON_DELETE_ERROR_ORDER').$coupon->order_id);
		}else{
		    if (!$table->delete($cid)){
                        $this->setError($table->getError());
                        return false;
