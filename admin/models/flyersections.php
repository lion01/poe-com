...
/**
 * Flyer Sections List Model
 */
class PoecomModelFlyerSections extends JModelList{
    ...
    */
    protected function getListQuery(){
        ...
        $q->select('fs.*, f.title flyer_title');
        $q->from('#__poe_flyer_section as fs');
        $q->innerJoin('#__poe_flyer f ON f.id=fs.flyer_id');

        // Filter by published state
        $published = $this->getState('filter.published');
        if (is_numeric($published)) {
                $q->where('fs.published = ' . (int) $published);
        }else if ($published === '') {
                $q->where('(fs.published = 0 OR fs.published = 1)');
        }
        ...
        $q->order('f.title, fs.ordering '.$dir);
