 ...
 */
class PoecomModelPayMethods extends JModelList{
    ...
    */
    protected function getListQuery(){
        ...
        $q->select( $this->getState('list.select','pm.*,ex.enabled,ex.extension_id'));
        $q->from('#__poe_payment_method AS pm');
        $q->innerJoin('#__extensions ex ON ex.element=pm.plugin');

        // Filter by pm_enabled state
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                $q->where('pm.id = '.(int) substr($search, 3));
            }else{
